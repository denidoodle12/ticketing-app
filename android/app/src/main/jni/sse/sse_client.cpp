...
void flush_event() {
    if (!g_event_name.empty() || !g_event_data.empty()) {
        LOGI("dispatch event='%s' data_len=%zu",
        ...
        if (g_on_event) {
            ...
            LOGW("g_on_event is null; dropping event '%s'",
                 g_event_name.c_str());
        }
    }
    g_event_name.clear();
...
size_t write_cb(char* ptr, size_t size, size_t nmemb, void* /*userdata*/) {
    ...
    if (!g_connected_emitted.load() && n > 0) {
        ...
        LOGI("stream established (first bytes received)");
...
void worker_main() {
    ...
    while (!g_stop_requested.load()) {
        ...
        LOGI("connecting (attempt %d) -> %s", attempt + 1, url_copy.c_str());
        ...
        if (!g_line_buf.empty()) {
            LOGI("flushing trailing line on disconnect: %s",
                 g_line_buf.c_str());
            handle_line(g_line_buf);
            g_line_buf.clear();
        }
        ...
        if (g_stop_requested.load()) {
            LOGI("stop requested; exiting worker");
            emit_status(SSE_STATUS_DISCONNECTED, "stopped");
            break;
        }

        if (http_code == 401) {
            LOGW("HTTP 401 — token invalid/expired");
            ...
            if (!g_token_updated.load()) {
                LOGE("No new token within 30s; giving up");
        ...
        LOGW("connection ended (CURLcode=%d, http=%ld, %s)",
        ...
        if (http_code == 200) {
            delay_s = 1;
            attempt = 0;
            LOGI("server closed mid-stream; quick reconnect in %ds", delay_s);
        } else {
            delay_s = 2 << std::min(attempt, 5);
            if (delay_s > 60) delay_s = 60;
            ++attempt;
            LOGI("reconnecting in %ds (attempt %d)", delay_s, attempt);
        }
...
void start(const char* url,
    ...
    if (g_running.load()) {
        LOGW("start requested but worker already running; ignoring");
        return;
    }
    if (!url || !token) {
        LOGE("start called with null url/token");
        return;
    }
    ...
    LOGI("start url=%s (token length=%zu, ca=%s)",
         url, strlen(token),
    ...
    try {
        ...
        LOGE("failed to spawn worker thread: %s", e.what());
        g_running.store(false);
    }
}
...
void update_token(const char* token) {
    ...
    LOGI("token updated (length=%zu)", strlen(token));
}

void stop() {
    if (!g_running.load()) {
        LOGI("stop requested but worker not running");
        return;
    }
    LOGI("stop requested");
